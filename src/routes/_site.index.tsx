...
export const Route = createFileRoute("/_site/")({
  head: () => ({
    meta: [
      ...
      { property: "og:image", content: BANNERS[0].image },
    ],
  }),
  component: HomePage,
});
  ...
  return (
    <div>
      <section className="relative h-[88vh] min-h-[600px] overflow-hidden">
        ...
        <div className="container-luxe relative z-10 flex h-full items-end pb-20 text-white">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="max-w-2xl">
            <p className="text-xs uppercase tracking-[0.3em] text-white/80">Autumn / Winter 2026</p>
            <h1 className="mt-4 font-display text-5xl font-medium leading-[1.05] sm:text-6xl lg:text-7xl">Crafted for the<br />cooler months.</h1>
            <p className="mt-4 max-w-md text-base text-white/90">A new chapter of timeless silhouettes, considered fabrics, and the quiet luxury of essentials made to outlast trends.</p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link to="/products"><Button size="lg" className="bg-white text-foreground hover:bg-white/90">Shop Collection <ArrowRight className="ml-2 h-4 w-4" /></Button></Link>
              <Link to="/collection/$slug" params={{ slug: "atelier-series" }}><Button size="lg" variant="outline" className="border-white/40 bg-transparent text-white hover:bg-white/10">Atelier Series</Button></Link>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <Section title="Shop by Category" subtitle="Curated essentials">
          ...
          {CATEGORIES.slice(0, 8).map((c) => (
            <Link key={c.id} to="/category/$slug" params={{ slug: c.slug }} className="group relative overflow-hidden rounded-md aspect-[4/5]">
              ...
              <div className="absolute inset-x-0 bottom-0 p-4 text-white">
                ...
                <p className="text-xs text-white/80">{c.productCount} pieces</p>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section className="container-luxe py-16">
        <div className="mb-8"><p className="text-xs uppercase tracking-wider text-muted-foreground">Featured</p><h2 className="mt-2 font-display text-3xl">Collections</h2></div>
          ...
          {COLLECTIONS.map((c) => (
            <Link key={c.id} to="/collection/$slug" params={{ slug: c.slug }} className="group relative block overflow-hidden rounded-md aspect-[3/4]">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white">
                ...
                <p className="mt-1 max-w-xs text-sm text-white/80">{c.description}</p>
      ...
      <section className="relative my-12 h-[420px] overflow-hidden">
        ...
        <div className="container-luxe relative z-10 flex h-full items-center text-white">
          <div className="max-w-xl">
            <h2 className="font-display text-4xl sm:text-5xl">Free worldwide shipping</h2>
            <p className="mt-3 text-white/90">On all orders over $150. Express delivery available at checkout.</p>
      ...
      <section className="container-luxe py-16">
        <div className="grid gap-4 md:grid-cols-3">
          <div className="md:col-span-2 aspect-[16/10] overflow-hidden rounded-md"><img src={CATEGORIES[0].image} className="h-full w-full object-cover" alt="Lookbook" /></div>
          <div className="flex flex-col justify-center">
            ...
            <Link to="/collection/$slug" params={{ slug: "autumn-edit" }} className="mt-6"><Button variant="outline">View Lookbook</Button></Link>
          </div>
        </div>
      </section>
      ...
      <section className="bg-secondary/40">
        <div className="container-luxe grid gap-10 py-16 md:grid-cols-2">
          <div>
            ...
            <p className="mt-4 text-muted-foreground">Founded in 2018, MAISON began as a small atelier with a single belief: that an everyday wardrobe should be made to last. Today our pieces are crafted in family-run workshops across Italy, Portugal, and Japan.</p>
            <Link to="/about" className="mt-6 inline-block"><Button variant="outline">Read More</Button></Link>
          </div>
          <div className="aspect-[4/3] overflow-hidden rounded-md"><img src={COLLECTIONS[2].image} className="h-full w-full object-cover" alt="Atelier" /></div>
        </div>
      </section>
      ...
      <section className="container-luxe py-20 text-center">
        ...
        <form className="mx-auto mt-6 flex max-w-md gap-2" onSubmit={(e) => e.preventDefault()}>
          <Input type="email" required placeholder="Enter your email" /><Button type="submit">Subscribe</Button>
        </form>
      </section>
    </div>
  );
